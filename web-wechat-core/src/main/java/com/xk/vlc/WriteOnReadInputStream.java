...
public abstract class WriteOnReadInputStream extends InputStream {
	...
	public WriteOnReadInputStream(InputStream source, long length, ICallback<Double> proceCall)  {
		if(null == source) {
	 ...
	 * @author kui.xiao
	 */
	public void init(File target) throws IOException {
		if(!target.exists()) {
	...
	public void close() throws IOException {
		...
		super.close();
		System.out.println("super closed");
		this.closed = true;
	}
			...
			public void run() {
				...
				try {
					while(!closed && (len = source.read(buffer, 0, buffer.length))>= 0) {
						...
						//缓存够了要通知
						if(buffered - read.position() > getBufferLimit()) {
							lock.lock();
							try {
								cond.signalAll();
							} catch (Exception e) {
