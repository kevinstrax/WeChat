 ...
 *******************************************************************************/
package com.xk.ui.swt.common.uiLib;
...
import com.xk.core.bean.StringNode;
import com.xk.ui.swt.items.ContactItem;
 ...
 */
public class XLabel extends Canvas {
	...
	/** the current text */
	private java.util.List<StringNode> text;
	/** the current icon */
	private Image image;
 ...
 * @see #getStyle()
 */
public XLabel(Composite parent, int style) {
 ...
 */
private Point getTotalSize(Image image, java.util.List<StringNode> text) {
	...
	if (text != null && text.size() > 0) {
		for(StringNode node : text) {
			if(node.type == 0) {
				Point e = gc.textExtent(node.base, DRAW_FLAGS);
				size.x += e.x;
 ...
 */
public java.util.List<StringNode> getText() {
	//checkWidget();
	return text;
}
@Override
...
private void initAccessible() {
	...
	accessible.addAccessibleListener(new AccessibleAdapter() {
		...
		public void getKeyboardShortcut(AccessibleEvent e) {
			...
			char mnemonic = _findMnemonic(sb.toString());	
			if (mnemonic != '\0') {
...
void onPaint(PaintEvent event) {
	...
	java.util.List<StringNode> t = text;
	Image img = image;
	...
	GC gc = event.gc;
	java.util.List<StringNode>[] lines = text == null ? null : splitString(text); 
 ...
 * </ul>
 */
public void setText(java.util.List<StringNode> text) {
	checkWidget();
	if (text == null) {
		text = new ArrayList<StringNode>(); //$NON-NLS-1$
		text.add(new StringNode(0, ""));
	}
 ...
 */
protected java.util.List<StringNode> shortenText(GC gc, java.util.List<StringNode> t, int width) {
...
int validateOffset(TextLayout layout, int offset) {
	...
	private java.util.List<StringNode>[] splitString(String text, int type) {
		java.util.List<StringNode>[] lines = new java.util.List[1];
		int start = 0, pos;
		do {
			pos = text.indexOf('\n', start);
			if (pos == -1) {
				java.util.List<StringNode> nodes = new ArrayList<StringNode>();
				StringNode node = new StringNode(0, text.substring(start));
				nodes.add(node);
				lines[lines.length - 1] = nodes;
			} else {
				boolean crlf = (pos > 0) && (text.charAt(pos - 1) == '\r');
				StringNode node = new StringNode(0, text.substring(start, pos
						- (crlf ? 1 : 0)));
				java.util.List<StringNode> nodes = new ArrayList<StringNode>();
				nodes.add(node);
				lines[lines.length - 1] = nodes;
				start = pos + 1;
				java.util.List<StringNode>[] newLines = new java.util.List[lines.length + 1];
	...
	private java.util.List<StringNode>[] splitString(java.util.List<StringNode> text) {
		java.util.List<StringNode>[] lines = new java.util.List[1];
		for(StringNode node : text) {
			if(lines[lines.length - 1] == null) {
				lines[lines.length - 1] = new ArrayList<StringNode>();
			}
			if(node.type == 0) {
				java.util.List<StringNode>[] temp = splitString(node.base,0);
				if(temp.length == 1) {
					lines[lines.length - 1].add(node);
				}else {
					lines[lines.length - 1].add(temp[0].get(0));
					java.util.List<StringNode>[] newLines = new java.util.List[lines.length + temp.length - 1];
					System.arraycopy(lines, 0, newLines, 0, lines.length);
					System.arraycopy(temp, 1, newLines, lines.length, temp.length - 1);
					lines = newLines;
				}
			}else {
				lines[lines.length - 1].add(node);
