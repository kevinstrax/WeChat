...
public class ConvItem extends ListItem {
    ...
    public ConvItem(ContactsStruct data, String name, String lastChat, String lastMsg, Long lastTime, boolean top,
            boolean silence, Integer unread) {
        super();
        this.data = data;
    ...
    public void draw(GC gc, int start, int width, int index) {
        ...
        Image showHead = headDefault;
        gc.drawImage(showHead, 0, 0, showHead.getImageData().width, showHead.getImageData().height, 15, start + 7, 50,
                50);
        ...
        if (null != lastMsg) {
            ...
            temp.addString(FileUtils.getLimitString(FileUtils.getLimitString(lastMsg, 7), 10), 15 + 58f, start + 37F,
                    font);
            gc.fillPath(temp);
            gc.setBackground(fg);
            temp.dispose();
