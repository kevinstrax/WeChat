...
public class QierSource implements IDownloadSource {
    ...
    public List<SearchInfo> getMV(String name) {
        ...
        for (Map<String, Object> map : list) {
            ...
            info.type = "mv";
            info.swfUrl = "https://imgcache.qq.com/tencentvideo_v1/playerv3/TPout.swf?max_age=86400&v=" + System
                    .currentTimeMillis();
        ...
        for (Map<String, Object> map : list) {
            SearchInfo info = new SearchInfo() {
                ...
                public String getUrl() {
                    ...
                    String text = HTTPUtil.getInstance()
                            .getHtml("https://c.y.qq.com/base/fcgi-bin/fcg_musicexpress.fcg?json=3&guid=" + guid);
                    ...
                    lrcUrl = String
                            .format("https://c.y.qq.com/lyric/fcgi-bin/fcg_query_lyric_new.fcg?callback=MusicJsonCallback_lrc&pcachetime=%s&songmid=%s&g_tk=5381&jsonpCallback=MusicJsonCallback_lrc&loginUin=0&hostUin=0&format=jsonp&inCharset=utf8&outCharset=utf-8&notice=0&platform=yqq&needNewCode=0",
                                    System.currentTimeMillis() + "", url);
                    url = String
                            .format("http://dl.stream.qqmusic.qq.com/M800%s.mp3?vkey=%s&guid=%s&fromtag=30", url, vkey,
                                    guid);
                    headers = new Header[]{new BasicHeader("referer", "https://y.qq.com/portal/player.html")};
                    urlFound = true;
                    return url;
    ...
    private Map<String, Object> fast(String name) {
        ...
        try {
            url = String
                    .format("https://c.y.qq.com/splcloud/fcgi-bin/smartbox_new.fcg?is_xml=0&format=jsonp&key=%s&g_tk=5381&jsonpCallback=%s&loginUin=0&hostUin=0&format=jsonp&inCharset=utf8&outCharset=utf-8&notice=0&platform=yqq&needNewCode=0",
                            URLEncoder.encode(name, "UTF-8"), callBack);
